...
- 納入**官方報告與統計**（美國 GAO 稽核報告、CBO 成本估計、普查局貿易統計、聯準會、中經院研究）
...
| 方式 | 說明 |
|------|------|
| **建立主題時自動展開** | 有 Groq 金鑰時由 LLM 產生；沒有金鑰時用內建的**領域詞庫**展開，一樣會產生多個關鍵字 |
| **♻️ 重新產生關鍵字** | 依主題名稱重新展開，**與現有關鍵字合併**，不會蓋掉你手動加的 |
| **💡 從實際新聞建議** | 試抓一次這個主題的新聞，統計實際出現的高頻詞彙，勾選後加入 |

領域詞庫涵蓋半導體、關稅、出口管制、供應鏈、碳邊境、AI、能源、風電、稀土、
匯率、通膨、專利、資安、生技、農業、貿易協定、反傾銷、強迫勞動，以及主要國家與區域。
例如輸入「半導體出口管制」會自動補上晶片、晶圓、先進製程、台積電、實體清單、
`semiconductor`、`export control`、`entity list` 等 20 個關鍵字。

「從實際新聞建議」是資料驅動的，冷門主題也有用。它會：

- 只採計**出現在 2 篇以上不同報導**的詞彙，濾掉單篇文章的特有名詞
- 濾掉既有關鍵字的片段（已有「半導體」時不會再建議「導體」「半導」）
- 遇到重疊的詞取較長的那個（不會建議「積電」而漏掉「台積電」）
- 太籠統的詞（台灣、中國、科技⋯）仍會列出，但**不預設勾選** ——
...
左側「**📥 下載紀錄**」頁可以把累積下來的每日新聞匯出成一份 Word 檔。

1. **選期間** —— 直接拖曳日曆選起訖日，或按「最近 7 天／30 天／90 天／全部」。
   可選範圍會自動夾在實際有資料的日期區間內。
2. **選主題** —— 預設全選，只列出這段期間真的有抓到新聞的主題。
3. **調內容**（可選）—— 只收錄官方來源、是否附摘要／重點條列／各日則數總表、
   每個主題每天最多幾則、日期由新到舊或由舊到新。
4. 按「📄 產生 Word 檔」，再按下載。

匯出的 `.docx` 包含封面（期間、天數、則數、官方來源數）、各日則數總表、
每天一節、每則新聞的**可點擊標題連結**、來源標記、發布時間、摘要與原始網址，
中文字型已設定為微軟正黑體，用 Word、Google 文件、LibreOffice 開都正常。

同一頁下方另有「下載原始 JSON（zip）」，把該區間的快照原檔打包，供後續程式處理。
